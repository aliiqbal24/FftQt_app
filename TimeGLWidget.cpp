...
TimeGLWidget::TimeGLWidget(QWidget *parent)
    : QOpenGLWidget(parent)
{
}

void TimeGLWidget::setTimeData(const QVector<double> &x, const QVector<double> &y)
{
    m_x = x;
    m_y = y;
    update();
...
void TimeGLWidget::initializeGL()
{
    initializeOpenGLFunctions();
    glClearColor(0.05f, 0.05f, 0.05f, 1.0f);
}

void TimeGLWidget::resizeGL(int w, int h)
{
    Q_UNUSED(w);
    Q_UNUSED(h);
}

void TimeGLWidget::paintGL()
{
    ...
    if (!m_x.isEmpty() && m_x.size() == m_y.size()) {
        for (int i=0;i<m_x.size(); ++i) {
            ...
            double py = height() - ((y - m_y.first())/(m_y.last()-m_y.first()+1e-9))*height();
