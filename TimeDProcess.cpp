...
void TimeDProcess::start() {
    ...
    connect(&workerThread, &QThread::started, this, [this]() {
        ...
        if (!time_buffer) resize(dynamic_time_buffer_size);

        ri_init();
        dev = ri_open_device();
        if (!dev) {
            qWarning("[TimeDProcess] Unable to find RI device");
            return;
        }

        static int64_t dummy = 0;
        ri_start_continuous_transfer(dev, TimeDProcess::transferCallback, &dummy);  // blocking
    });
...
void TimeDProcess::resize(int size) {
    ...
    uint16_t* new_buffer = (uint16_t*)calloc(size, sizeof(uint16_t));
    if (!new_buffer) {
        pthread_mutex_unlock(&time_mutex);
        fprintf(stderr, "[TimeDProcess] Failed to allocate time buffer\n");
        return;
    }
...
int TimeDProcess::transferCallback(uint16_t *data, int ndata, int dataloss, void *) {
