...
void Features::togglePause(bool &isPaused) {
    isPaused = !isPaused;
    qDebug() << "[Features] Pause toggled. isPaused =" << isPaused;
}

void Features::switchMode(FFTMode &mode) {
    ...
    qDebug() << "[Features] Mode switched to:"
             << (mode == FFTMode::FullBandwidth ? "FullBandwidth" : "LowBandwidth");
}

void Features::saveFFTPlot(const QString &fileName,
                           const double *fftBuffer, double sampleRate)
{
    QFile file(fileName);
...
void Features::saveTimePlot(const QString &fileName,
                            ...
                            double sampleRate, double timeWindowSeconds)
{
    ...
    for (int i = 0; i < static_cast<int>(buffer.size()); ++i) {
        ...
        double power_uW = (static_cast<double>(buffer[i]) - offset) * scale;
...
void Features::promptUserToSavePlot(QWidget *parent,
                                    const double *fftBuffer,
                                    const std::vector<uint16_t> &timeBuffer)
{
    QSettings settings("Ultracoustics", "RealtimePlotApp");
    QString lastDir = settings.value("lastSavePath", QDir::homePath()).toString();

    QString fileName = QFileDialog::getSaveFileName(parent, "Save Plot Data", lastDir,
